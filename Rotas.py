...
def adiciona_player(id_jogo: int, player_id: int):
    ...
    if player_id in jogo_atual.lista_jogadores:
        raise HTTPException(status_code=400, detail="Jogador já foi adicionado")
    
    jogador_adicionado = jogo_atual.add_jogador(player_id)
    if not jogador_adicionado:
        raise HTTPException(status_code=400, detail="Não foi possível adicionar o jogador. Cores ou objetivos esgotados.")
